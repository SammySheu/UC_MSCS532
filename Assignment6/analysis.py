...
Assignment 6 - Deterministic Selection (Median of Medians) vs Randomized Quickselect
"""
import random
import sys
...
from selection import median_of_medians_select, randomized_quickselect, measure_time


class SelectionAnalyzer:
    """Analyzer for comparing Median-of-Medians and Randomized Quickselect."""
    ...
    def run_comparison(self, sizes: List[int], data_types: List[str], trials: int = 5):
        """
        Run a comprehensive comparison of deterministic and randomized selection.

        For each configuration, k is set to n//2 (finding the median).
        ...
        """
        algorithms = {
            'Median of Medians': median_of_medians_select,
            'Randomized Quickselect': randomized_quickselect,
        }
        ...
        for data_type in data_types:
            ...
            for size in sizes:
                ...
                k = size // 2  # select the median element

                for algo_name, algo_func in algorithms.items():
                    times = []
                    for _ in range(trials):
                        data = self.generate_test_data(size, data_type)
                        t = measure_time(algo_func, data, k)
                        times.append(t)
    ...
    def plot_results(self, sizes: List[int], data_types: List[str]):
        ...
        """
        colors = {
            'Median of Medians':     '#E53935',
            'Randomized Quickselect': '#1E88E5',
        }
        markers = {
            'Median of Medians':     's',
            'Randomized Quickselect': 'o',
        }

        df = pd.DataFrame(self.detailed_results)

        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Selection Algorithm Comparison: Median of Medians vs Randomized Quickselect',
        ...
        plt.tight_layout()
        plt.savefig('./Assignment6/selection_comparison.png', dpi=300, bbox_inches='tight',
                    facecolor='white', edgecolor='none')
        plt.show()
        print("Chart saved to 'selection_comparison.png'")

    def export_to_csv(self, filename: str = 'selection_results.csv'):
...
def main():
    ...
    print("Median of Medians vs Randomized Quickselect")
    print("=" * 55)
    ...
    analyzer.export_to_csv('./Assignment6/selection_results.csv')
